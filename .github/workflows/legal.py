...
def shell_style(text):
    return _format(text, prefix='# ')


ap = argparse.ArgumentParser()
ap.add_argument('--repair', action='store_true', default=False)
args = ap.parse_args()
with open('COPYRIGHT.txt', 'rt') as f:
    cr = f.read()
with open('LICENSE.txt', 'rt') as f:
    lic = f.read()
dl = cr.find('2015-20')
cr = cr.replace(cr[dl : dl + 9], '2015-{}'.format(datetime.datetime.now().year))
formatters = {
    '.hamlbars': _format,
    '.hbs': _format,
    '.html': _format,
...
for p, dns, fns in os.walk('.'):
    if any([p.startswith('./.{}'.format(d)) for d in ['cache', 'git']]):
        continue
