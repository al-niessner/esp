...
class Handler(socketserver.BaseRequestHandler):
    def _real_file(self, fn):
        ...
        _send(self.request, json.dumps({'done': True, 'err': '', 'fn': fn}))
        with open(fn, 'br') as f:
            _xfer(self.request, size, f)
        return
        ...
        if not os.path.exists(msg['fn']):
                ...
                json.dumps(
                    {
                        ...
                        'err': 'file "'
                        + msg['fn']
                        + '" is a directory and recuse if False',
                        'fn': msg['fn'],
                    }
                ),
            )
            ...
            if os.path.isdir(msg['fn']):
                for dp, dns, fns in os.walk(msg['fn'], followlinks=True):
                    for fn in fns:
                        self._real_file(os.path.join(dp, fn))
                    pass
                _send(
                    self.request,
                    json.dumps(
                        {'done': True, 'err': 'All files copied', 'fn': fn}
                    ),
                )
            else:
                self._real_file(msg['fn'])
            pass
        return
...
def client(port: int, recurse: bool, filenames: [str]):
    if len(filenames) < 2:
        ...
        for fn in filenames[:-1]:
            ...
            while (
                not (response['done'] and response['fn'] == fn)
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(
        description='A cheat tool to copy files into a building container during the run statement'
    )
    ap.add_argument(
        'filenames',
        default=[],
    ...
    if args.server:
        server(args.filenames)
    else:
        client(args.port, args.recursive, args.filenames)
    pass
