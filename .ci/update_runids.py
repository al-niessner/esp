...
def compress_rid_notation (c2rid:{str:[int]})->[(int, str, str)]:
    ...
    for cs,rids in cs2rid.items():
        if len (rids) == 1: srid = str(list(rids)[0])
        else: srid = '{0}:{1}'.format (min (rids), max (rids))
...
def extract_from_log (lfn:str)->[(int,str)]:
    result = []
    with open (lfn, 'rt') as f: lines = f.readlines()
...
def fetch_existing_messages (mfn:str)->{str:str}:
    result = {}
    with open (mfn) as f: content = f.read()
...
def write (mfn:str, messages:{str:str})->None:
    ...
    for message in sorted (messages, reverse=True):
        ...
        pass
    block.append ('')
    with open (mfn) as f: markdown = f.read()
    ...
    with open (mfn, 'tw') as f: f.write (markdown[:start] +
                                         '\n'.join (block) +
                                         markdown[end:])
    return
...
if __name__ == '__main__':
    ...
    for fn in sorted (filter (lambda s,lp=args.log_prefix:s.startswith (lp),
        ...
        for rid,cs in extract_from_log (os.path.join (args.log_dir, fn)):
            if cs in cs2rid: cs2rid[cs].add (rid)
            else: cs2rid[cs] = set([rid])
            pass
        pass
